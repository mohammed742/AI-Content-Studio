...
- **Current sub-task**: **STU-C5 (DEV-67) — Carousel asset kits (multi-image) — DONE + committed `b85562e` + pushed to `origin/main` + DEV-67 → Done (2026-07-22)**. Human QA'd in-browser (gallery slideshow + guard rails). Added multi-image **Carousel** Asset Kits: `asset_kits` gained an ordered `media` jsonb array (`AssetKitMedia[] = {url, mediaType}`, default `[]`; applied to Neon via direct DDL). New `src/lib/carousel.ts` (`CarouselService.assemble`: validate 2–10 frames → download each frame in order → R2 upload with index-ordered keys → save ONE kit whose `media` order is the source of truth; cover `mediaUrl`/`mediaType` mirror frame 0 so single-image consumers are untouched; non-fatal embed-back). `POST /api/carousel` (auth + Zod, min/max frames). Gallery read extended (`GET /api/gallery` select + `GalleryItem` now carry the ordered `media` array). Gallery UI: card **layers badge** w/ frame count + lightbox **ordered slideshow** (prev/next, dots, `n/N` marker, per-slide download). Storage = jsonb array over child table (matches `content_plans.items`/`hashtags`; zero migration risk). `CAROUSEL_MIN_FRAMES=2`/`MAX=10` live in `@/lib/carousel` (not schema — the Node test runner can't resolve `@/db/schema` for runtime *value* imports, only type-only). **`typecheck ✅ · tests 147/147 ✅ (+8) · lint ✅ · build ✅` (`/api/carousel` registers).** Left **In Progress** (board has no Needs Review — same as STU-C4/C6). **Not committed yet** — awaiting human go-ahead. Prior: STU-C1, C2, C3, C4, C6 Done.
- **Prior sub-task**: **STU-C6 (DEV-63) — Seasonal + industry strategy data — DONE + committed `debfa48` + DEV-63 → Done (2026-07-21)**. Made the seasonal calendar **region-aware** and added **per-industry `strategyHint`** to the planner, extending the existing TS modules rather than creating the plan's `data/*.json`. `seasonal.ts`: `Region` type (US/UK/CA/AU) + `GLOBAL` set merged per region via `holidaysForRegion(region)`; `upcomingHolidays(now, windowDays, region = "US")` (repurposed the never-used injectable `holidays` param — US merged set byte-identical to old `HOLIDAYS`, so all prior tests pass). `industry-templates.ts`: added `strategyHint` string to all 8 templates (`Record<enum>` exhaustiveness). `content-planner.ts`: optional `PlannableProfile.region` seam (defaults US) threaded into `upcomingHolidays`, `strategyHint` injected into `buildPlannerPrompt`. **No schema change, no migration, no UI.** `typecheck ✅ · tests 139/139 ✅ (+6) · lint ✅ · build ✅`. Left **In Progress** (board has no Needs Review — same as STU-C4). **Not committed yet** — awaiting human go-ahead. Prior: STU-C1, STU-C2, STU-C3, STU-C4 Done.
...
- **UI work**: **yes** (STU-C5 — gallery card badge + lightbox slideshow viewer; browser visual QA is human-gated by Clerk auth, left as a QA checklist item per the DEV-26/STU-C3 UI-verification pattern — no component test harness exists in this repo). Prior: STU-C6 data/lib only; STU-C4 service+API; STU-C3 UI (`/library`).
- **Files modified this session (STU-C5, 2026-07-22)**:
