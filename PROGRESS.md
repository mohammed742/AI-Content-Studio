...
- **Deferred (carryovers, not this slice)**: (1) live end-to-end video render — gated on placeholder presenter portraits + the still-503-unverified DEV-32 assembly model; built against the real services with injectable seams + unit tests instead. (2) Commit/push of DEV-33 — built on the working tree, left uncommitted per the human ("build on top of the working tree"), awaiting review.
