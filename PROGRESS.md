...
- **Current sub-task**: **STU-C3 (DEV-62) — Media Library — DONE (human-verified upload works, 2026-07-18)**. New `media_library` table + `MediaLibraryService` (magic-byte-validated uploads) + **`/api/media-library` routes (GET/POST/DELETE)** + `/library` page (grid/upload/delete, transparent-input overlay). `typecheck ✅ · tests 124/124 ✅ · lint ✅ · build ✅`; DDL applied to Neon + round-trip verified; storage path live-verified (R2+DB). **Note:** upload was re-architected from Server Actions → `/api` + `fetch` (matches Gallery); the long "upload does nothing" saga was ultimately a **browser file-dialog block** on the user's Chrome (worked in another browser), not an app bug. Prior: STU-C1, STU-C2 Done.
- **Next action**: Commit STU-C3 (this session). Next unblocked slices: **STU-C4 (DEV-66)** now unblocked (C3 Done — before/after composite), **STU-C6 (DEV-63)**, **STU-C7 (DEV-64)** (blocker-free); **STU-C5 (DEV-67)**. ⚠️ Carried: text-graphic premium legibility completion-gated by the `nano-banana-2` shim; confirm STU-C1 `flux-krea-dev` repoint; replace `OPENAI_API_KEY` if still 401. Phase 3 first slice = DEV-27. Next unblocked slices: **STU-C6 (DEV-63)**, **STU-C7 (DEV-64)** (blocker-free); **STU-C5 (DEV-67)** (C2 Done). **STU-C4 (DEV-66)** now unblocked by STU-C3 (once C3 → Done). ⚠️ **Completion-gated**: text-graphic legibility at premium tier (`nano-banana-pro`) can only be verified once the key serves more than `nano-banana-2` — endpoints are live, but the free-tier shim still collapses generation to `nano-banana-2`. ⚠️ Also still open: confirm the STU-C1 `social_graphic.premium`→`flux-krea-dev` repoint. Carried: replace revoked `OPENAI_API_KEY` if still 401. Phase 3 first slice remains DEV-27. Deferred: DEV-60 (PostHog onboarding events, Phase 8).
...
- **UI work**: **yes** (STU-C3 — `/library` page, DESIGN §5/§7)
