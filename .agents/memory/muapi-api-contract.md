...
**2026-08-05 (UGC smoke-test) — REAL full-length data point, dynamic-pricing caveat confirmed.** Ran the actual pipeline through `infinitetalk-image-to-video` with a real ~15s gemini voiceover (not the short DEV-29 test clip) + a 480p portrait PNG (`image_url` verified public + raster). Completed to a real 2.2 MB `video/mp4`, but: **render time ≈260s (4.3 min)** and **actual cost $0.96 (76 credits)** — ~3.4× the DEV-29 probe's $0.28/91.5s, and **over the plan's $0.80/video alert.** So the "$0.20 catalog" and "$0.28 probe" figures badly understate a real full-length talking head; budget ~$1/video for the lip-sync step alone. **Two consequences for the UGC pipeline: (1)** the model's real runtime (~260s) exceeds any sane inline HTTP poll budget — the pipeline's `MuapiService` default `pollTimeoutMs` is 60s and even a temporary 180s bump timed out; **the real fix is background execution + client status-polling/webhooks, not a longer inline timeout** (holding one request open across talking-head ~260s + B-roll ~75s + assembly + reframe is ~10min, impossible on serverless). **(2)** a **faceless monogram placeholder still rendered** (Muapi didn't reject it, `has_nsfw_contents:false`) — so the portrait carryover isn't blocking *a* render, but a usable talking head still needs a real face. See PROGRESS.md 2026-08-05 session log + [[presenter-portraits-phase3]].
