...
- **Pushing to remote is allowed.** After a slice is committed, agents may `git push origin main` (fast-forward only). **Do not force-push or rewrite already-pushed history**, and never push in AFK/night-shift mode — pushing is an interactive-mode action only, done at the human's request or after their go-ahead. The human still reviews all changes.
