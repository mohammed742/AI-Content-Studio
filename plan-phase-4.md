...
**Status**: In progress — STU-31 (DEV-35) built 2026-07-28 (connect flow + `social_accounts`); STU-32 (DEV-34) built 2026-07-28 (accounts management — rename + disconnect); STU-33 (DEV-36) built 2026-07-29 (publish-to-YouTube — `publish_jobs` + publish/poll/retry service + `POST`/`GET /api/social/publish` + publish UI on `/social`); STU-34 (DEV-37) built 2026-07-30 (publish-to-TikTok — `tiktok-publish` params builder + platform dispatch in `publishAssetKit` + account-driven publish UI); STU-35 (DEV-38) built 2026-08-02 (publish-to-Instagram — `instagram-publish` params builder + Instagram branch in the platform dispatcher + Instagram in the account-driven publish UI); STU-36 (DEV-39) built 2026-08-02 (publishing history — full history table on `/social` with thumbnails, status badges, expandable rows + retry; pure display helpers; no schema/service/route changes). **All 6 Phase-4 slices built.**
**Goal**: Connect social accounts and publish content directly from the app.

> ⚠️ **Contract note (verified live 2026-07-28, DEV-35):** Muapi's social API is real and matches `muapi.ai/docs/social-publishing`. `POST /social/{youtube|tiktok|instagram}/connect-url` → `{ url }`; `GET /social/ext/accounts?external_user_id=…` → account array; publish endpoints `POST /{platform}-publish` (price is per-platform: `youtube-publish` $0.01, `tiktok-publish` $0.02, `instagram-publish` $0.02 — all verified live). **Muapi owns the OAuth callback itself** and only bounces the user to our `redirect_to` afterward, so our side stores **no OAuth tokens** — just the `muapiAccountId`. Instagram uses `instagram_business_*` scopes (Business/Creator account required). `platform` stored as a string enum, not the plan's "1/2/3".
...
- `STU-34`: Publish-to-TikTok flow — **BUILT (DEV-37, 2026-07-30)**: `tiktok-publish` params builder (`buildTikTokPublishParams` + `resolvePrivacyLevel`) + `PublishJobParams` split into a per-platform union + `publishAssetKit` dispatch by platform + account-driven publish UI (one card spanning YouTube+TikTok). **Contract verified live**: `tiktok-publish` slug, **$0.02** (2× YouTube), required `account_id`(int)/`media_url`; optional caption(≤150)/`privacy_level`(4-enum)/`allow_comment`/`allow_duet`/`allow_stitch`/`is_ai_generated` (defaults true). Static checks green; live publish to a real account is human-gated QA (⚠️ unaudited TikTok apps may be `SELF_ONLY`-only until approved).
- `STU-35`: Publish-to-Instagram flow — **BUILT (DEV-38, 2026-08-02)**: `InstagramPublishParams` added to the `PublishJobParams` union (jsonb-type only, no migration) + `buildInstagramPublishParams`/`resolvePlacement` on `social-publishing.ts` + Instagram branch in the `buildPublishParams` dispatcher (the "not available yet" throw is gone) + Instagram in the account-driven publish UI (caption + placement select + share-to-feed toggle). **Contract verified live**: `instagram-publish` slug, **$0.02**, required `account_id`(int)/`media_url`; optional `caption`(≤2200)/`placement`(reels/stories/timeline)/`share_to_feed`. Static checks green; live publish needs a real IG Business/Creator account (human-gated QA).
- `STU-36`: Publishing history + status tracking — **BUILT (DEV-39, 2026-08-02)**: full **Publishing History** table on `/social` (`src/components/social/publish-history.tsx`) — every job newest-first with an asset thumbnail (video first-frame + icon fallback), platform, relative date, status badge (Processing/Published/Failed), and an expandable row (live post URL, error, published time, Retry). Pure display helpers in `src/lib/publish-history.ts` (`summarizePublishJobs`/`formatRelativeTime`/`statusMeta`), TDD'd. **No schema/service/route changes** — reuses `listPublishJobs` (advance-on-read) + `GET /api/social/publish`; jobs + the 5s poll stay owned by `<SocialPublish>` (no double-fetch). Static checks green; authed visual QA is human-gated. **Gallery "Publish" button deferred** (publish-flow plumbing, not history). **Last Phase-4 slice.**
